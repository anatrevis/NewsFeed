...
import { fetchKeywords, fetchArticles, fetchSummarizeStatus, Keyword, Article, SummarizeStatus } from '../services/api'
...
export default function Home() {
  ...
  const [summarizeStatus, setSummarizeStatus] = useState<SummarizeStatus | null>(null)
  ...
  // Check summarize status on mount
  useEffect(() => {
    const checkSummarizeStatus = async () => {
      try {
        const status = await fetchSummarizeStatus()
        setSummarizeStatus(status)
      } catch (err) {
        console.error('Failed to check summarize status:', err)
        setSummarizeStatus({ enabled: false, message: 'Unable to check AI summarization status' })
      }
    }
    checkSummarizeStatus()
  }, [])
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative group">
                <button
                  ...
                  disabled={articlesLoading || articles.length === 0 || !summarizeStatus?.enabled}
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                    summarizeStatus?.enabled
                      ? 'bg-gradient-to-r from-accent-cyan to-accent-pink text-white hover:opacity-90'
                      : 'bg-midnight-800 border border-midnight-600 text-slate-400'
                  }`}
                >
                  <svg
                    className="w-4 h-4"
                ...
                {!summarizeStatus?.enabled && summarizeStatus?.message && (
                  <div className="absolute z-50 bottom-full right-0 mb-2 w-64 p-3 bg-midnight-800 border border-midnight-600 rounded-xl shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                    <p className="text-sm text-slate-400">{summarizeStatus.message}</p>
                    <div className="absolute bottom-0 right-4 translate-y-1/2 rotate-45 w-2 h-2 bg-midnight-800 border-r border-b border-midnight-600"></div>
                  </div>
                )}
              </div>
